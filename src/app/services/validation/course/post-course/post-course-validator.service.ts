...
export class PostCourseValidatorService {
  ...
  validate(data: PostCourseDTO): CourseValidation {
    ...
    if (data.dates.length === 0) {
      ...
      validationObject.datesError='Es muss mindestens ein Datum für einen Kurs existieren';
      result = false;
    }
    if (data.duration < 1) {
      console.error('Ein Kurs muss länger als 0 Minuten dauern');
      validationObject.durationError='Ein Kurs muss länger als 0 Minuten dauern';

      result = false;
    }
    if (data.numberParticipants < 1) {
      console.error('Es die Teilnehmeranzahl muss mindestens 1 sein');
      validationObject.numberOfParticipantsError='Es die Teilnehmeranzahl muss mindestens 1 sein';

      result = false;
    }
    if (data.numberWaitlist < 1) {
      console.error('Es muss mindestens einen Wartelistenplatz geben');
      validationObject.numberWaitlistError='Es muss mindestens einen Wartelistenplatz geben';

      result = false;
    }
    if (data.prices.length === 0) {
      console.error('Ein Kurs muss mindestens einen Preis haben');
      validationObject.pricesError='Ein Kurs muss mindestens einen Preis haben';

      result = false;
    }
    if (!data.location) {
      console.error('Ein Kurs muss einem Ort zugeordnet sein');
      validationObject.locationError='Ein Kurs muss einem Ort zugeordnet sein';

      result = false;
    }
    if (!data.meetingPoint) {
      console.error('Ein Kurs muss ein Treffpunkt zugeordnet werden');
      validationObject.meetingPointError='Ein Kurs muss ein Treffpunkt zugeordnet werden';
    ...
    if (!data.notes) {
      ...
      validationObject.notesError='Es muss eine Notiz zu einem Kurs existieren';
